...
@retry(
    ...
    retry=retry_if_exception(
        lambda e: isinstance(e, LLMError) and _should_retry_exception(e)
    ),
    reraise=True,
    ...
    try:
        with httpx.Client(timeout=timeout_sec) as client:
            ...
            if response.status_code == 429:
                ...
                raise LLMApiError(
                    f"Gemini client error (HTTP {response.status_code}): {response.text}"
                )
        ...
        raise LLMTimeoutError(
            f"Gemini API call timed out after {timeout_sec}s"
        ) from exc
    ...
    try:
        with httpx.Client(timeout=timeout_sec) as client:
            ...
            if response.status_code == 429:
                ...
                raise LLMApiError(
                    f"OpenAI client error (HTTP {response.status_code}): {response.text}"
                )
            ...
            try:
                ...
                raise LLMApiError(
                    f"Malformed OpenAI API response: {data}"
                ) from parse_err

    except httpx.TimeoutException as exc:
        raise LLMTimeoutError(
            f"OpenAI API call timed out after {timeout_sec}s"
        ) from exc
    except httpx.RequestError as exc:
